...
- (UIButton *)doneBtn{
    if (!_doneBtn) {
        ...
        [rightBtn setTitleColor:[UIColor colorWithHexString:@"#4680ff"] forState:UIControlStateNormal];
...
- (UICollectionView *)toolBarThumbCollectionView{
    if (!_toolBarThumbCollectionView) {
        ...
        // CGRectMake(0, 22, 300, 50)
        UICollectionView *toolBarThumbCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(10, 0, self.view.width - 100, 50) collectionViewLayout:flowLayout];
...
- (ZLPhotoPickerCollectionView *)collectionView{
    if (!_collectionView) {
        ...
        collectionView.backgroundColor = KQYCColor(ffffff, 1e1e1e);
        // 时间置顶
...
- (UILabel *)makeView{
    if (!_makeView) {
        ...
        makeView.frame = CGRectMake(35, 0, 15, 15);
        makeView.hidden = YES;
        makeView.layer.cornerRadius = makeView.frame.size.height / 2.0;
        makeView.clipsToBounds = YES;
        makeView.backgroundColor = [UIColor colorWithHexString:@"4680ff"];
    ...
    [[ZLPhotoPickerDatas defaultPicker] getGroupPhotosWithGroup:self.assetsGroup finished:^(NSArray *assets) {
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            [assets enumerateObjectsUsingBlock:^(ALAsset *asset, NSUInteger idx, BOOL *stop) {
                    ZLPhotoAssets *zlAsset = [[ZLPhotoAssets alloc] initWithALAsset:asset];
...
//    NSString *heightVfl = @"V:[toorBar(50)]-0-|";
...
//    self.selectAssets = [NSMutableArray arrayWithArray:[[NSSet setWithArray:self.selectAssets] allObjects]];
    ...
    if (self.selectPickerAssets.count || deleteAssets) {
        ...
        for (NSInteger i = 0; i < self.selectAssets.count; i++) {
            ...
            if([[asset.assetURL absoluteString] isEqualToString:[photoAsset.assetURL absoluteString]]){
...
- (void)photoBrowser:(ZLPhotoPickerBrowserViewController *)photoBrowser removePhotoAtIndexPath:(NSIndexPath *)indexPath{
    ...
    for (NSInteger i = 0; i < self.collectionView.dataArray.count; i++) {
        ...
        if ([photoAsset isKindOfClass:[ZLPhotoAssets class]] && [asset isKindOfClass:[ZLPhotoAssets class]]) {
                ...
                if([[assets.assetURL absoluteString] isEqualToString:[photoAssets.assetURL absoluteString]]){
                    currentPage = i;
                    break;
                }
            }
