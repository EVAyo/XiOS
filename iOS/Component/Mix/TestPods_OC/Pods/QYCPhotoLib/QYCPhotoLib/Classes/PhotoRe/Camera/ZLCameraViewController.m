...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        layout.sectionInset                = UIEdgeInsetsMake(0, ZLCameraColletionViewPadding * 0.5, 0, 0);
...
- (void)setup {
    ...
    topView.frame           = CGRectMake(0, 0, self.view.width, 44);
    ...
    NSURL *BudleURL  = [bundle URLForResource:@"QYCPhotoLib" withExtension:@"bundle"];
    NSBundle *Budle  = [NSBundle bundleWithURL:BudleURL];
    UIImage *image   = [UIImage imageNamed:@"QYCPhotoLib_camear_light_close" inBundle:Budle compatibleWithTraitCollection:nil];
    // set
    ...
    _doneBtn.frame = CGRectMake(self.view.width - 2 * margin - width, 0, width, controlView.height);
    [_doneBtn setTitle:@"确定" forState:UIControlStateNormal];
    [_doneBtn setTitleColor:[UIColor colorWithHexString:@"#4680ff"] forState:UIControlStateNormal];
    [_doneBtn addTarget:self action:@selector(doneAction) forControlEvents:UIControlEventTouchUpInside];
    [controlView addSubview:_doneBtn];
    _doneBtn.hidden = YES;
...
- (void)deleteImageView:(ZLCameraImageView *)imageView {
    ...
    self.doneBtn.hidden = self.images.count == 0;
}
...
- (void)showPickerVc:(UIViewController *)vc {
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        if (status == AVAuthorizationStatusDenied || status == AVAuthorizationStatusRestricted) {
            [QYCToast showToastWithMessage:@"你没有相机的访问权限?\n前往 设置->隐私->相机 打开权限" type:QYCToastTypeDefault];
            return;
        }
        else {
        ...
        [QYCToast showToastWithMessage:@"你的设备不支持拍摄" type:QYCToastTypeDefault];
    }
}
                                                    ...
                                                    completionHandler:
                                                        ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
                                                            ...
                                                            if (imageSampleBuffer == nil) {
                                                                [QYCToast showToastWithMessage:@"手机没来的及处理相片，请重拍！" type:QYCToastTypeDefault];
                                                                return;
                                                            }
                                                            ...
                                                            NSString *path = [[NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:currentTimeStr];
...
- (void)flashCameraDevice:(UIButton *)btn {
    // 取当前bundle图片
    NSBundle *bundle = [NSBundle bundleForClass:[self class]];
    NSURL *BudleURL  = [bundle URLForResource:@"QYCPhotoLib" withExtension:@"bundle"];
    NSBundle *Budle  = [NSBundle bundleWithURL:BudleURL];
    ...
    if (_isOpen == NO) {
        ...
        UIImage *image = [UIImage imageNamed:@"QYCPhotoLib_camear_light" inBundle:Budle compatibleWithTraitCollection:nil];
        // set
...
- (void)stillImage:(id)sender {
    ...
    if (self.maxCount > 0 && self.images.count >= self.maxCount) {
        ...
        [QYCToast showToastWithMessage:[NSString stringWithFormat:@"拍照的个数不能超过%ld", (long)self.maxCount] type:QYCToastTypeWarning];
...
- (UIImage *)fixOrientation:(UIImage *)srcImg {
    ...
    CGContextRef ctx = CGBitmapContextCreate(NULL, width, height, CGImageGetBitsPerComponent(srcImg.CGImage), 0, CGImageGetColorSpace(srcImg.CGImage), CGImageGetBitmapInfo(srcImg.CGImage));
