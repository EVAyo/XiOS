...
- (void)longGesture:(UILongPressGestureRecognizer *)gesture{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        if (!self.sheet) {
            self.sheet = [UIAlertController alertControllerWithTitle:@"提示" message:nil preferredStyle:UIAlertControllerStyleActionSheet];
            [self.sheet addAction:[UIAlertAction actionWithTitle:@"保存到相册" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
            ...
            [self.sheet addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
                
            }]];
        }
        UIPopoverPresentationController *popover = [self.sheet popoverPresentationController];
        if (popover){
            popover.sourceView = self;
            popover.sourceRect = CGRectMake([UIScreen mainScreen].bounds.size.width * 0.5, [UIScreen mainScreen].bounds.size.height, 1.0, 1.0);
            popover.permittedArrowDirections = UIPopoverArrowDirectionAny;
        }

        [self.viewController presentViewController:self.sheet animated:YES completion:^{}];
    }
}
...
- (void)setPhoto:(ZLPhotoPickerBrowserPhoto *)photo{
    ...
    if (photo.photoURL.absoluteString.length) {
        ...
        if (photoRange.location != NSNotFound){
            ...
            // 网络URL
            [_photoImageView sd_internalSetImageWithURL:photo.photoURL placeholderImage:thumbImage options:SDWebImageHandleCookies operationKey:nil setImageBlock:nil progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
