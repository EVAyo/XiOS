    ...
    if (self.isRecoderSelectPicker){
        ...
        for (ZLPhotoAssets *asset in self.selectAssets) {
            for (ZLPhotoAssets *asset2 in self.dataArray) {
                ...
                if ([asset.assetURL isEqual:asset2.assetURL]) {
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.item == 0 && self.topShowPhotoPicker){
        ...
        if (self.isRecoderSelectPicker) {
            for (ZLPhotoAssets *asset in self.selectAssets) {
                if ([asset isKindOfClass:[ZLPhotoAssets class]] && [asset.assetURL isEqual:[(ZLPhotoAssets *)self.dataArray[indexPath.item] assetURL]]) {
...
- (BOOL)validatePhotoCount:(NSInteger)maxCount{
    if (self.selectAssets.count >= maxCount) {
        ...
        [QYCToast showToastWithMessage:format type:QYCToastTypeWarning];
        return NO;
    }
    return YES;
}
