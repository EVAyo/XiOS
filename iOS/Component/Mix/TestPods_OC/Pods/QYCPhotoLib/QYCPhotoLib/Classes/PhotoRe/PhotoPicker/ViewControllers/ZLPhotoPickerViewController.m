...
- (void)showPickerVc:(UIViewController *)vc{
    ...
    if (weakVc != nil) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [[tbc selectedViewController] presentViewController:self animated:YES completion:nil];
        });
...
- (void)done:(NSNotification *)note{
    NSArray *selectA =  note.userInfo[@"selectAssets"];
    NSMutableArray *selectArray = [NSMutableArray arrayWithArray:selectA];
    dispatch_async(dispatch_get_main_queue(), ^{
