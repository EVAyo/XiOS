...
- (void)delete{
    ...
    QYAlterView *alert = [[QYAlterView alloc] initWithAlterViewStyle:QYAlertViewStyleStyleDefault title:@"确定要删除此图片？" cancelButtonTitle:@"取消"otherButtonTitle:@"删除"];
    
    __weak typeof(self)weakSelf = self;
    alert.sureBtnBlock = ^(NSString *text){
        __strong typeof(weakSelf)strongSelf = weakSelf;
        
        
        NSInteger page = self.currentPage;
        if ([self.delegate respondsToSelector:@selector(photoBrowser:removePhotoAtIndexPath:)]) {
            [self.delegate photoBrowser:strongSelf removePhotoAtIndexPath:[NSIndexPath indexPathForItem:page inSection:strongSelf.currentIndexPath.section]];
        }
        
        if (strongSelf.photos.count > strongSelf.currentPage || strongSelf.dataSource != nil) {
            NSMutableArray *photos = [NSMutableArray arrayWithArray:strongSelf.photos];
            [photos removeObjectAtIndex:strongSelf.currentPage];
            strongSelf.photos = photos;
        }
        
        if (page >= strongSelf.photos.count) {
            strongSelf.currentPage--;
        }
        
        strongSelf.status = UIViewAnimationAnimationStatusFade;
        UICollectionViewCell *cell = [strongSelf.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:page inSection:strongSelf.currentIndexPath.section]];
        if (cell) {
            if([[[cell.contentView subviews] lastObject] isKindOfClass:[UIView class]]){
                ...
                [UIView animateWithDuration:.35 animations:^{
                    ...
                    [QYCToast showToastWithMessage:@"删除成功" type:QYCToastTypeSuccess];
                }];
            }
        }
