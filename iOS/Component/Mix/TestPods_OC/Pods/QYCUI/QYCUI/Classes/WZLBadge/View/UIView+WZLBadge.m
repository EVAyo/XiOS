...
- (void)showBadgeWithStyle:(WBadgeStyle)style value:(NSInteger)value animationType:(WBadgeAnimType)aniType {
    ...
    if (@available(iOS 13.0, *)) {
        if ([NSStringFromClass(self.class) containsString:@"UIBadgeVie"]) {//兼容iOS13下tUItabbarItem
           self.badge.center = CGPointMake(5 + 2 + self.badgeCenterOffset.x, self.badgeCenterOffset.y + 5);
        }
    }
...
- (void)badgeInit {
    if (self.badgeBgColor == nil) {
        self.badgeBgColor = HexColor(0xFF4C4C); // 统一红点颜色
    }
