...
@interface CMVideoRecordManager () <AVCaptureFileOutputRecordingDelegate, AVCapturePhotoCaptureDelegate>
// 媒体（音、视频）捕获会话
...
@implementation CMVideoRecordManager
...
- (NSTimer *)timer {
    if (!_timer) {
        _timer = [NSTimer scheduledTimerWithTimeInterval:KTimerInterval target:self selector:@selector(timerFired) userInfo:nil repeats:YES];
    }
    return _timer;
}
...
- (void)timerFired {
    ...
    if ([self.delegate respondsToSelector:@selector(recordTimeCurrentTime:totalTime:)]) {
        [self.delegate recordTimeCurrentTime:self.recordTime totalTime:self.KMaxRecordTime];
    }
    if (_recordTime >= self.KMaxRecordTime) {
...
- (void)compressVideo:(NSURL *)inputFileURL
    ...
    NSURL *outPutUrl = [NSURL fileURLWithPath:[CMVideoRecordManager cacheFilePath:NO]];
    [self convertVideoQuailtyWithInputURL:inputFileURL
                                outputURL:outPutUrl
                          completeHandler:^(AVAssetExportSession *exportSession) {
