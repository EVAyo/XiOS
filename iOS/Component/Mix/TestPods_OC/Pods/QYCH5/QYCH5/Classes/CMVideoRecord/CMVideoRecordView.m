...
- (void)initSubViews {
    ...
    _recorderManager.KMaxRecordTime = self.KMaxRecordTime;
...
- (UIImageView *)focusImageView {
    if (!_focusImageView) {
        UIImage *image        = [UIImage imageNamed:@"record_video_focus" aClass:[self class] bundle:QYCH5];
...
- (UIButton *)backButton {
    if (!_backButton) {
        ...
        UIImage *image = [UIImage imageNamed:@"record_video_back" aClass:[self class] bundle:QYCH5];
...
- (UIButton *)switchCameraButton {
    if (!_switchCameraButton) {
        ...
        UIImage *image      = [UIImage imageNamed:@"record_video_camera" aClass:[self class] bundle:QYCH5];
        [_switchCameraButton setImage:image forState:UIControlStateNormal];
        //        _switchCameraButton.frame = CGRectMake(self.view.frame.size.width - 20 - 28, 40, 30, 28);
        _switchCameraButton.frame = CGRectMake(self.view.frame.size.width - 60 - 36, HEIGHT_H5 - 60 - 36, 30, 28);
...
- (void)saveVideo {
    if (UIVideoAtPathIsCompatibleWithSavedPhotosAlbum([self.recordVideoUrl path])) {
        //保存视频到相簿
        UISaveVideoAtPathToSavedPhotosAlbum([self.recordVideoUrl path], self, @selector(video:didFinishSavingWithError:contextInfo:), nil);
    }
}
...
- (void)compressVideo {
    ...
    [self.recorderManager compressVideo:self.recordVideoUrl
                               complete:^(BOOL success, NSURL *outputUrl) {
...
- (void)takePhotoCompletedWithImage:(UIImage *)image error:(NSError *)error {
    ...
    playView.confirmBlock = ^{
        //首先,需要获取沙盒路径
        NSString *picPath = [self getCurrentImagePath];

        BOOL success = [UIImageJPEGRepresentation(image, 1.0) writeToFile:picPath atomically:YES];
        if (success && weakSelf.photoCompletionBlock) {
            weakSelf.photoCompletionBlock(image, picPath);
            [weakSelf dismissViewControllerAnimated:YES completion:NULL];
            NSLog(@"写入本地成功");
        }
        else {
...
- (NSString *)getCurrentImagePath {
    ...
    NSString *picPath    = [NSString stringWithFormat:@"%@/Documents/%@.png", NSHomeDirectory(), timeString];
    return picPath;
}
